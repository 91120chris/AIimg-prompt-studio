...
- The FLUX install endpoint downloads `FLUX_MODEL_REPO_ID` into `FLUX_MODEL_LOCAL_DIR` when `HF_TOKEN` has access. The default is `black-forest-labs/FLUX.2-klein-9b`, because this repo contains the Diffusers `model_index.json` pipeline structure. The `diffusers_flux2` image provider supports Phase 2B T2I with width, height, steps, guidance, and seed. I2I remains Phase 2C.
- The `black-forest-labs/FLUX.2-klein-9b-fp8` and `black-forest-labs/FLUX.2-klein-9b-kv-fp8` repos are single-file checkpoint downloads. This provider currently rejects those folders with a clear message instead of passing them to `DiffusionPipeline.from_pretrained`.
- FLUX.2 Klein 9B is large; the model card notes around 29GB VRAM for the 9B model. `FLUX_DEVICE_MAP=balanced` is the default. Use `cpu` for CPU placement, or set `cuda` only if you want the app to load first and then call `.to("cuda")`; `cuda` is not passed as a Diffusers `device_map` strategy.
