...
- LoRA 掛載：支援從 ComfyUI `loras` 資料夾動態選擇 LoRA 模型並調整權重（-5 到 +5）。
...
將 `.safetensors` 格式的 LoRA 檔案放入 `ComfyUI/models/loras/` 資料夾，重啟 ComfyUI 後即可在 app 的 LoRA 下拉選單中選用。
...
1. 將 LoRA `.safetensors` 檔案放入 `ComfyUI/models/loras/`。
2. 確認 ComfyUI 已啟動且 backend 已連線（圖像 Provider 選 `Local Flux`）。
3. 上方工具列的 **LoRA** 下拉選單會自動列出可用 LoRA（後端呼叫 ComfyUI `/models/loras` 取得）。
4. 選擇 LoRA 後，**權重** 滑桿出現，範圍 -5 到 +5（預設 0.8）。
   - 正值：加強 LoRA 風格效果。
   - 負值：抑制 LoRA 特徵（anti-LoRA）。
5. 按 `確認生成`，後端會在 ComfyUI workflow 中插入 `LoraLoader` 節點，自動接線 UNETLoader 與 CLIPLoader。
6. 若不需要 LoRA，選回 **None** 即可，workflow 不會插入任何 LoRA 節點。
