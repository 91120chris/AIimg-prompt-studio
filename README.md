...
- The FLUX install endpoint downloads `FLUX_MODEL_REPO_ID` into `FLUX_MODEL_LOCAL_DIR` when `HF_TOKEN` has access. The `diffusers_flux2` image provider supports Phase 2B T2I with width, height, steps, guidance, and seed. I2I remains Phase 2C.
- FLUX.2 Klein 9B FP8 is large; the model card notes around 29GB VRAM for the 9B model. `FLUX_DEVICE_MAP=balanced` is the default. Use `cpu` for CPU placement, or set `cuda` only if you want the app to load first and then call `.to("cuda")`; `cuda` is not passed as a Diffusers `device_map` strategy.
