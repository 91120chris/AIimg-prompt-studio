...
- FLUX.2 Klein 9B FP8 is large but substantially lighter than the BF16 transformer. `FLUX_DEVICE_MAP=balanced` is the portable default. On a CUDA-enabled PyTorch install, set `FLUX_DEVICE_MAP=cuda` to load first and then call `.to("cuda")`; `cuda` is not passed as a Diffusers `device_map` strategy.
