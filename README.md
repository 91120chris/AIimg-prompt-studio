...
- FLUX.2 Klein 9B FP8 is large but substantially lighter than the BF16 transformer. `FLUX_DEVICE_MAP=balanced` is the recommended CUDA setup on a 4090 because it uses GPU placement with CPU offload when needed. `FLUX_DEVICE_MAP=cuda` tries to move the loaded pipeline onto CUDA directly and may run out of VRAM.
