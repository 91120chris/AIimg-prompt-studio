...
Phase 2A has started: the FLUX manager can read `HF_TOKEN`, report safe Hugging Face readiness, install the configured FLUX snapshot with `huggingface_hub`, classify gated/private/token/download errors, and register only a safe path label for the frontend.
...
- The FLUX install endpoint downloads `FLUX_MODEL_REPO_ID` into `FLUX_MODEL_LOCAL_DIR` when `HF_TOKEN` has access. T2I execution is still the next Phase 2B task.
