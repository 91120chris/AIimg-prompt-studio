...
def test_codex_agent_turn_can_disable_persistent_session(monkeypatch, tmp_path) -> None:
    from app.api import agent

    questionnaire = Questionnaire(
        questionnaire_id="q_codex_non_persistent",
        title="Questions",
        questions=[
            TextQuestion(
                kind="text",
    ...
    FakeCodexRunner.next_codex_session_id = "019e8426-new-session"
    monkeypatch.setattr(agent, "CodexAgentRunner", FakeCodexRunner)
    ...
    with new_session(app.state.engine) as db:
        db.add(
            CodexConversationRecord(
                session_id=session_id,
                codex_session_id="019e8426-existing-session",
            )
        )
        db.commit()
    ...
    response = client.post(
        ...
        json={
            ...
            "original_prompt": "glass cup",
            "provider": "codex_cli",
            "use_persistent_codex_session": False,
        },
    )

    assert response.status_code == 200
    assert FakeCodexRunner.codex_session_ids == [None]
    with new_session(app.state.engine) as db:
        record = db.get(CodexConversationRecord, session_id)

    assert record is not None
    assert record.codex_session_id == "019e8426-existing-session"


def test_reset_codex_conversation_deletes_saved_session(tmp_path) -> None:
    app, client = make_test_app(tmp_path)
    session_id = client.post("/sessions", json={"title": "Test"}).json()["session_id"]
    with new_session(app.state.engine) as db:
        db.add(
    ...
    response = client.delete(f"/agent/codex-conversation/{session_id}")
