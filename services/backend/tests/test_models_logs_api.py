...
def make_flux_checkpoint_file(tmp_path, name: str = "private-flux-fp8.safetensors"):
    model_dir = tmp_path / "local_models" / name
    model_dir.parent.mkdir(parents=True, exist_ok=True)
    model_dir.write_bytes(b"fake")
    return model_dir
...
def test_flux_status_and_set_path_do_not_return_raw_model_path(tmp_path) -> None:
    ...
    model_path = str(make_flux_checkpoint_file(tmp_path, "flux-secret-path.safetensors"))
...
def fake_flux_install(tmp_path):
    def _install(_settings) -> FluxInstallResult:
        model_path = make_flux_checkpoint_file(tmp_path, "downloaded-flux.safetensors")
...
def test_flux_readiness_does_not_return_hf_token_or_raw_path(tmp_path) -> None:
    ...
    model_path = str(make_flux_checkpoint_file(tmp_path))
    ...
    assert payload["path_configured"] is True
    assert payload["path_label"] == "private-flux-fp8"
...
def test_flux_actions_preserve_path_label_and_write_safe_logs(monkeypatch, tmp_path) -> None:
    ...
    assert "FLUX model path selected: private-flux-fp8" in logs.text
